...
function loadOldDossier (idClient) {
    myDBKitadi.transaction(function (transaction) {
        transaction.executeSql(`SELECT * FROM Client WHERE Client_Id = ${idClient}`, [], function (tx, results) {
            let len = results.rows.length, i;
            for (i = 0; i < len; i++) {       
    ...
    myDBKitadi.transaction(function (transaction) {
        transaction.executeSql(`SELECT * FROM Client WHERE Id = ${idClient}`, [idClient], function (tx, results) {
            let len = results.rows.length, i;
            console.log(len);
            memoryI = len;
            for (i = 0; i < len; i++) {  
