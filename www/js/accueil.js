        ...
        transaction.executeSql('SELECT * FROM Client ORDER BY DteVisite DESC', [], function (tx, results) {
            ...
            for (i = 0; i < len; i++) {
                ...
                <tr id="${idClientRow}">
                    ...
                    <td> <button id="buttonLoad_${idClientRow}"> Modifier </button> </td>
                </tr>
                `
                
                let btnLoadId = "buttonLoad_" + idClientRow.toString()
                btnLoad[i] = document.getElementById(btnLoadId);
                btnLoad[i].addEventListener("click", function() {
                    console.log("Click!");
                    loadDossier()
                })
...
function loadOldDossier (id_client) {
    myDBKitadi.transaction(function (transaction) {
        var executeQuery = "SELECT * FROM Client WHERE Id=?";
        transaction.executeSql(executeQuery, [id_client], function (tx, results) {
    
    header.innerHTML = `
    <h2>Informations client</h2>
        <p class="mb-2 text-gray-500 dark:text-gray-400">
        <h3>Nom</h3>
        <input type="text" id="nomClient" value="${results.rows.item(0).Nom}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>Prénom</h3>
        <input type="text" id="prenomClient" value="${results.rows.item(0).Prenom}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>Adresse</h3>
        <input type="text" id="adresseClient" value="${results.rows.item(0).Adresse}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>Code Postal</h3>
        <input type="text" id="codePostal" value="${results.rows.item(0).CodPostal}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>Ville</h3>
        <input type="text" id="villeClient" value="${results.rows.item(0).Ville}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>Téléphone</h3>
        <input type="text" id="telClient" value="${results.rows.item(0).Tel}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
        <h3>e-mail</h3>
        <input type="text" id="mailClient" value="${results.rows.item(0).Mail}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500"/>
    </p>`
